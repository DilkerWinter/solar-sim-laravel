...
export default function EnergyInfoCard({ energyInfo, isEditing }) {
  ...
  const resumo = `Consumo médio: ${editedEnergyInfo.average_monthly_consumption_kwh} kWh/mês – R$ ${editedEnergyInfo.average_energy_bill}`;
    ...
    <div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-gray-800">
          ...
          <h2 className="text-sm font-medium">Informações de Energia Elétrica</h2>
        </div>
      ...
      {!isOpen && (
        <p className="mt-2 text-sm text-gray-700">{resumo}</p>
      )}
          ...
          {isEditing ? (
            <>
              <EditableField
                ...
                value={editedEnergyInfo.average_monthly_consumption_kwh}
                onChange={(e) => handleEnergyInfoChange("average_monthly_consumption_kwh", e.target.value)}
              />
              <EditableField
                label="Consumo Médio Anual (kWh)"
                name="average_annual_consumption_kwh"
                value={editedEnergyInfo.average_annual_consumption_kwh}
                onChange={(e) => handleEnergyInfoChange("average_annual_consumption_kwh", e.target.value)}
              />
              <EditableField
                label="Conta de Energia Média (R$)"
                name="average_energy_bill"
                value={editedEnergyInfo.average_energy_bill}
                onChange={(e) => handleEnergyInfoChange("average_energy_bill", e.target.value)}
              />
              <EditableField
                label="Concessionária de Energia"
                name="energy_provider"
                value={editedEnergyInfo.energy_provider}
                onChange={(e) => handleEnergyInfoChange("energy_provider", e.target.value)}
              />
              <EditableField
                label="Observações"
                name="notes"
                value={editedEnergyInfo.notes || ""}
                onChange={(e) => handleEnergyInfoChange("notes", e.target.value)}
                multiline
              />
            </>
          ) : (
